namespace :users do
  desc 'Backfill users.created_at from tmp/user_creations.csv (set DRY_RUN=true to preview)'
  task backfill_timestamps: :environment do
    ...
    CSV.foreach(path, headers: true).with_index(1) do |row, idx|
      ...
      # case-insensitive match on username first
      user = nil
      if username && !username.empty?
        user = User.where('lower(username)=?', username.downcase).first
      end

      # fallback: parse SQL-values in row['line'] to find email or username
