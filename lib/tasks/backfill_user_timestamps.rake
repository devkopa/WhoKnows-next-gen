namespace :users do
  ...
  task backfill_timestamps: :environment do
    ...
    entries.each do |key, ts|
      ...
      if user
        if ENV["DRY_RUN"]=="true"
          dry_outputs << "DRY: would set user #{user.id}/#{user.username} created_at => #{ts}"
        else
    ...
    if ENV["DRY_RUN"]=="true"
      dry_outputs.uniq!
      dry_outputs.sort_by! do |s|
        s[/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/]
      end
      dry_outputs.each { |l| puts l }
      puts "DRY: Unique matches: #{dry_outputs.size}, Skipped keys: #{skipped}"
    else
